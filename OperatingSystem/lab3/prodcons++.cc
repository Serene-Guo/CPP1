...
#define BUFF_SIZE 5  // the size of the round buffer
#define N_PROD    2 // the number of producers 
#define N_CONS    3  // the number of consumers
#define N_MESSG   6  //   the number of messages produced by each producer
...
Consumer(_int which)
{
 ...
 //ntf("consumer\n");
      ...
      // ...
            ...
            nfull->P();  //full 要减一  如果 full 为 0  则 挂 在  待队列上 阻塞
       mutex->P();//  要对 ring进行操作，所以先 wait 互斥信号
...
nfull=new Semaphore("full",0);
    ...
    //BUFF_SIZE here.
    // ...    
ring=new Ring(BUFF_SIZE);  // ring.cc 文件里 
...
producers[i]->Fork(Producer,i);         //传的 参数 是i 标识 不同的生产者  
    ...
    for (i=0; i < N_CONS; i++) 
    {
      ...
      //  ...
consumers[i] = new Thread(cons_names[i]);///  参数 名字 不知道对不对 ？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
