...
{ 
    hdr = new FileHeader;
    hdr->FetchFrom(sector); //  bitHdr->FetchFrom(FreeMapSector);
...
{//  自己添加
 ...
 bool OpenFile::AppFileSize(int filesize,bool half)
{//自己添加的 
                 OpenFile *freeMapFile = new OpenFile(FreeMapSector);
                 BitMap *freeMap =new BitMap(NumSectors); // NumSectors 
	         freeMap->FetchFrom(freeMapFile);
     if(!(hdr->AppSector(freeMap,filesize,half))) 
          { 
            printf("append too big file!!!!!!!!!!!----------openfile.cc文件中\n");
            return false;
           }
...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    //  printf("\n????????????%d\n",fileLength);  //128   200
    firstSector = divRoundDown(position, SectorSize);
    lastSector = divRoundDown(position + numBytes-1, SectorSize);  //少写了个－1， 若为position 为0，numBytes为 128 则 last=1,numSectors=2,而 实际上 只需要 1 个 扇区，,
// 这时，当 从hdr 中 读dataSectors 时，  会多度一个，读出的是 0，这样 覆盖掉了  0号扇区。。。。。 
    numSectors = 1 + lastSector - firstSector;
...
//printf("firstAligned %d,lastAligned %d\n",firstAligned,lastAligned);
...
// write modified sectors back  接下来 把 buff中的 内容写回对应的扇区 
    for (i = firstSector; i <= lastSector; i++)	
{//printf("dataSectors[%d]",i);
