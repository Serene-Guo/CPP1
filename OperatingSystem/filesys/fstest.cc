...
printf("修改hdr之前，文件长度%d\n ",openFile->Length());

    if (half) start = start / 2;
    openFile->Seek(start);
    
if(!(openFile->AppFileSize(fileLength,half)))
{// /扩展 该文件的长度  hdr 变化 
printf("append too big file!!!!!!!!!!!----------fstest.cc文件中\n");
return;
}

printf("修改hdr之后，文件长度%d\n ",openFile->Length());
...
openFile->WriteBack(to);   /// 其实 就是 hdr 写回 磁盘   
