...
GCoptimization::EnergyType GCoptimization::giveDataEnergyFnPix()
{
    ...
    EnergyType eng = (EnergyType) 0;
	EnergyType temp = (EnergyType) 0;

    for ( int i = 0; i < m_nPixels; i++ )
       {
		   temp=m_dataFnPix(i,m_labeling[i]);
		   eng = eng +temp ;
	//cout<<"label: "<<m_labeling[i]<<"  data_term:"<< temp<<endl;
	}
...
GCoptimization::EnergyType Expansion::start_expansion(int max_num_iterations )
{
    ...
    while (old_energy < 0||(old_energy > new_energy  && curr_cycle <= max_num_iterations) ) //
    {
		cout<<curr_cycle<<"  th while>>>>>>>>>>>>>>>>>>"<<endl;

        old_energy = new_energy;
        new_energy = oneExpansionIteration();
        cout<<"data energy("<<dataEnergy()<<")	smooth("<<smoothnessEnergy()<<")	new energy "<<new_energy<<endl;
        curr_cycle++;   
		cout<<"old energy: "<<old_energy<<" , new energy "<<new_energy<<endl;
		cout<<"next iter  condition: "<< (old_energy < 0 )<< " "<<(old_energy >new_energy) <<"  "<<(curr_cycle <= max_num_iterations)<<endl;
